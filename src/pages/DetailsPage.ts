...
export async function renderDetailsPage(params: Record<string, string>): Promise<void> {
  ...
  main.innerHTML = '';
  main.appendChild(
    h('div', { class: 'details-page animate-page-enter' },
  ...
  const hero = h('div', { class: 'details-hero' },
    ...
    h('div', { class: 'details-hero__content container' },
      h('div', { class: 'details-hero__poster' },
        h('img', {
  ...
  if (isSeries && series.seasons && series.seasons.length > 0) {
    const seasonsSection = h('div', { class: 'details-section container' },
      h('h2', { class: 'section__title' }, 'Seasons & Episodes')
    );

    for (const season of series.seasons) {
      const seasonBlock = h('div', { class: 'season-block glass-card' },
        h('div', { class: 'season-block__header' },
          h('h3', { class: 'season-block__title' }, season.title || `Season ${season.seasonNumber}`),
      ...
      if (season.episodes && season.episodes.length > 0) {
        ...
        for (const ep of season.episodes) {
          const epCard = h('a', {
            ...
            h('div', { class: 'episode-card__info' },
